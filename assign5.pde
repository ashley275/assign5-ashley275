...
void draw() {
  ...
  switch (gameState) {
    ...
    case GAME_RUN: // In-Game
      ...
      // Caution
      int enemyIndex = getEnemyIndexByRow(playerRow + 5);
      if(enemyIndex > -1) drawCaution(enemyIndex);
...
boolean isHit(float ax, float ay, float aw, float ah, float bx, float by, float bw, float bh){
  return (ax < bx+bw && ax+aw > bx && ay < by+bh && ay+ah > by) ? true : false;
}

String convertFramesToTimeString(int frames){
  int minute = floor(frames / 60) / 60;
  int second = floot(frames / 60) - minute * 60;
  return (second < 10) ? "0" + minute + ":0" + second : "0" + minute + ":" + second;
}

color getTimeTextColor(int frames){
  if(frames >= 7200) return #00ffff;
  else if(frames >= 3600) return #ffffff;
  else if(frames >= 1800) return #ffcc00;
  else if(frames >= 600) return #ff6600;
  else return #ff0000;
}
...
int getEnemyIndexByRow(int row){
  ...
  for(int i = 0; i < soldierY.length; i++){
    if(soldierY[i] == row * SOIL_SIZE) return i;
  }
  return -1;
}

void drawCaution(int enemyIndex){

  image(caution, soldierX[enemyIndex], soldierY[enemyIndex] - SOIL_SIZE);
